  ...
  render: function(){
    ...
    if(typeof this.model.operationsArray !== 'undefined'){
      ...
      for (var i = 0; i < this.model.operationsArray.length; i++) {
        ...
        var pathsWhitelist = this.options.swaggerOptions.whitelist.paths;
        if(!pathsWhitelist || (pathsWhitelist[operation.path] && pathsWhitelist[operation.path].indexOf(operation.method) > -1)) {
           this.addOperation(operation);
        }else{
          delete this.model.operationsArray[i];
        }
      }
    }
